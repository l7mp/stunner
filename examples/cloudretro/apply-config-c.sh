...
sed 's/CLOUD_GAME_WEBRTC_ICESERVERS_0_USERNAME: default/CLOUD_GAME_WEBRTC_ICESERVERS_0_USERNAME: '"$(kubectl get gatewayconfig -n stunner stunner-gatewayconfig -o jsonpath='{.spec.userName}')"'/' | \
sed 's/CLOUD_GAME_WEBRTC_ICESERVERS_0_CREDENTIAL: default/CLOUD_GAME_WEBRTC_ICESERVERS_0_CREDENTIAL: '"$(kubectl get gatewayconfig -n stunner stunner-gatewayconfig -o jsonpath='{.spec.password}')"'/' | \
kubectl apply -f -
