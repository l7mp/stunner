...
sed 's/CLOUD_GAME_WEBRTC_ICESERVERS_0_URL: NULL/CLOUD_GAME_WEBRTC_ICESERVERS_0_URL: turn:'"$(kubectl get service -n stunner stunner-gateway-udp-gateway-svc -o jsonpath='{.status.loadBalancer.ingress[0].ip}')"':3478/' | \
sed 's/CLOUD_GAME_WEBRTC_ICESERVERS_0_URL: NULL/CLOUD_GAME_WEBRTC_ICESERVERS_0_USERNAME: '"$(kubectl get gatewayconfig -n stunner stunner-gatewayconfig -o jsonpath='{.spec.userName}')"'/' | \
