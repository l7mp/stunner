...
# Download stunnerctl
tmp=$(mktemp -d /tmp/stunnerctl.XXXXXX)
NAME="stunnerctl-${STUNNER_VERSION}"
URL="${REPO}/releases/download/${STUNNER_VERSION}/stunnerctl-${STUNNER_VERSION}-${OSEXT}-${STUNNER_ARCH}"
filename="stunnerctl-${STUNNER_VERSION}-${OSEXT}-${STUNNER_ARCH}"
...
mv "${tmp}/${filename}" "$HOME/.l7mp/bin/stunnerctl"
chmod +x "$HOME/.l7mp/bin/stunnerctl"
rm -r "${tmp}"

# Print final message
printf "\n"
printf "Add stunnerctl to your path with:"
printf "\n"
printf "  export PATH=\$HOME/.l7mp/bin:\$PATH \n"
printf "\n"
printf "Need more information? Visit https://docs.l7mp.io/en/${STUNNER_VERSION}/cmd/stunnerctl/ \n"
