...
* [RFC 6062](https://tools.ietf.org/html/rfc6062) and [RFC
  8656](https://tools.ietf.org/html/rfc8656): Traversal Using Relays around NAT (TURN) Extensions
  for TCP Allocations
...
The daemon can be stated in one of two modes. For quick tests `stunnerd` can be configured as a
simple TURN server by specifying a TURN URI on the command line. For more complex scenarios, and
especially for use in a Kubernetes cluster, `stunnerd` can take configuration from a config
file. In addition, `stunnerd` implements a watch-mode, where it actively monitors the config file
for updates and, once the config file has changed, it automatically reconciles its internal state
to the new configuration. This mode is intended for use with the [STUNner Kubernetes gateway
operator](https://github.com/l7mp/stunner-gateway-operator): the operator watches Kubernetes
[Gateway API](https://gateway-api.sigs.k8s.io) resources and renders the current control plane
